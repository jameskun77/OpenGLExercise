...
EGLRender::EGLRender()
{
    ...
    m_SampleHandle = GL_NONE;
    m_TexSizeHandle = GL_NONE;
    m_FboId = GL_NONE;
...
void EGLRender::Init()
{
    ...
    m_SampleHandle = glGetUniformLocation(m_ProgramObj,"s_TextureMap");
    m_TexSizeHandle = glGetUniformLocation(m_ProgramObj,"u_texSize");
...
void EGLRender::SetIntParam(int paramType,int param)
{
    switch (paramType)
    {
...
void EGLRender::Draw()
{
    ...
    glBindTexture(GL_TEXTURE_2D,m_ImageTextureId);
    glUniform1f(m_SampleHandle,0);

    if(m_TexSizeHandle > -1)
    {
        GLfloat size[2];
        size[0] = m_RenderImage.width;
        size[1] = m_RenderImage.height;
        glUniform2f(m_TexSizeHandle,size[0],size[1]);
    }
