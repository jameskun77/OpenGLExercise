...
{
    m_TextureId = GL_NONE;
    m_FrameBuffId = GL_NONE;
    m_FrameBufferTextureId = GL_NONE;
    m_VaoIds[2] = {GL_NONE};
    m_VboIds[4] = {GL_NONE};
    m_sampleUniform = GL_NONE;
    m_FrameBufferProgramObj = GL_NONE;
    m_FboVertexShader = GL_NONE;
    m_FboFragmentShader = GL_NONE;
    m_FrameBufferSamplerUniform = GL_NONE;
}
...
void FrameBufferSample::Init()
{
    ...
    m_FrameBufferProgramObj = GLUtils::CreateProgram(vShaderStr,fFboShaderStr,m_FboVertexShader,
            m_FboFragmentShader);
    if (m_ProgramObj == GL_NONE || m_FrameBufferProgramObj == GL_NONE)
    {
        LOGCATE("FBOSample::Init m_ProgramObj == GL_NONE");
        return;
    }

    m_sampleUniform = glGetUniformLocation(m_ProgramObj,"s_TextureMap");
    m_FrameBufferSamplerUniform = glGetUniformLocation(m_FrameBufferProgramObj,"s_TextureMap");
...
void FrameBufferSample::Draw(int screenW, int screenH)
{
    ...
    glUseProgram(m_FrameBufferProgramObj);
    glBindVertexArray(m_VaoIds[1]);
    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_2D, m_TextureId);
    glUniform1i(m_FrameBufferSamplerUniform, 0);
    GO_CHECK_GL_ERROR();
...
void FrameBufferSample::Destroy()
{
    ...
    if (m_FrameBufferProgramObj)
    {
        glDeleteProgram(m_FrameBufferProgramObj);
    }
    ...
    if (m_FrameBuffId)
    {
        glDeleteFramebuffers(1, &m_FrameBuffId);
    }
}

bool FrameBufferSample::CreateFrameBufferObj()
{
